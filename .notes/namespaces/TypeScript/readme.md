...
所以，这跟[隔壁](../Python) Python 利用语法糖来实现这种 Env 作为 Self 位的导入是不一样的：隔壁那个 NS Yard 显然不会像这样冗余。

*这是两种实现思路的差异，利用不同语言语法来表达。这里的 Echoes 方案在那边应该也能做、那边的利用 OOP 语法糖语言特性的思路这边肯定也有办法做。但 TypeScript 的泛型表达很流畅、所以用来体现此 Echoes 方案；而 OOP 语法糖在 TypeScript 我又不太了解，所以那种方案就用 Python 做了。*
